...
# Framing the Problem
You can deal with a problem in two ways. First, the intuitive way: go at it and follow your common sense. In process, however, you'll discover recurring patterns. Some sub-problem pops up constantly or a technique you apply time after time. Hence the second way is to frame the problem. Give the recurring patterns names, describe how they relate to each other. This way, you develop the domain of the problem.
...
One recurring theme when dealing with Dotty issues is that you have a program which makes the compiler to exhibit some unwanted behavior. For example, a recent issue [#8188](https://github.com/lampepfl/dotty/issues/8188):
...
And it compiles successfully. The existence of the program very close to the original one that doesn't exhibit the undesired behavior is interesting because if the programs are close, the compiler must also do more-or-less the same thing compiling them.
...
## Deltas
An intuitive way to go about establishing the reason why the failing case exhibits the undesired behavior is to see what exactly the compiler did differently in case of the failing program. This way of reasoning is especially appealing because one such difference is immediately obvious: the undesired behavior itself, for example a compilation error, and the absence of it in the successful case.
...
## Tracers
The tracers are the statements that output something to the terminal. They are the primary way to detect the deltas.

Sometimes a single trace is output more than once. This can happen in case of recursive methods or when a method is called from a loop. It is often the case that the correct and failing programs' traces behave exactly the same way – up to a certain point.
...
Motivated by the above, the traces can be labelled by MD5 hashes that is computed from both the string being output as well as all the other strings that were output previously.
...
- Where (Above) – the line of code on which a tracer (a console output command) is injected. "Above" means "put the tracer on that line, above anything that was previously on that line".
- Trace – which exactly tracer, what does the program output at that point?
- Delta Successful – what is the tracer output for the successful program?
- Delta Failing – what is the tracer output for the failing program?

Defined that way, the deltas are self-consistent: you can reproduce them on a clean codebase using only that information.
