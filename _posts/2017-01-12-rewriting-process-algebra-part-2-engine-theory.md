...
TODO The last two sentences are not clear to me. You only explain trees later.

TODO I am also a bit confused by the suspension type because not every functor
satisfies the requirements. `Option` for example would not be a good fit, but
`Future` would be.

TODO How do suspended computations fit into the overall framework? They are not 
mentioned in the first blog article in the Theory section.
